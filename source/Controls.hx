...
class Controls extends FlxActionSet
{
	...
	#if !web var _reset = new FlxActionDigital(Action.RESET); #end
	...
	public function setKeyboardScheme(scheme:KeyboardScheme, reset = true)
	{
		...
		switch (scheme)
		{
			case Solo:
				...
				#if !web inline bindKeys(Control.RESET, [FlxKey.fromString("R")]); #end
			case Duo(true):
				...
				#if !web inline bindKeys(Control.RESET, [R]); #end
			case Duo(false):
				...
				#if !web inline bindKeys(Control.RESET, [BACKSPACE]); #end
			case None: // nothing
			case Custom: // nothing
		}
		#else
	...
	public function loadKeyBinds()
	{
		...
		inline bindKeys(Control.BACK, [BACKSPACE, ESCAPE]);
		inline bindKeys(Control.PAUSE, [P, ENTER, ESCAPE]);
		#if !web inline bindKeys(Control.RESET, [FlxKey.fromString(FlxG.save.data.killBind)]); #end
	}
